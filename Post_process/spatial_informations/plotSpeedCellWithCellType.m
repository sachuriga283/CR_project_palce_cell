function plotSpeedCellWithCellType(save_path,animalID,day_num, session_num, cellType)
...
for i=1:length(id)
    ...
    s = scatter(velocity(ten_point),smooth_spk(ten_point),1);
    s.Marker = "o";
    s.MarkerEdgeColor="none";
    s.MarkerFaceColor=[211/255 211/255 211/255];

    speedScores(i,:) = analyses.speedScore(velocity, itan_spk, 20);
    title(['speedscore: ' round(num2str(speedScores(i,1),2))],'Color','white');
    if speedScores(i,:) >= 0.3
        if cellType(i) == 0
            s.MarkerFaceColor='magenta';
            hold on
            p = plot(bin_point,p50,'r.');
            p.MarkerSize=10;
            p255 = plot(bin_point,p25,'r-');
            p755 = plot(bin_point,p75,'r-');
        else
            hold on
            p = plot(bin_point,p50,'r.');
            p.MarkerSize=10;
...
savefig(f1,[path 'speed_cell.fig'])
